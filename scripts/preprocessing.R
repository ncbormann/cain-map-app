...
setwd(dirname(getwd())) 
setwd("/home/ncb/Dropbox/Documents/ERC_DANGER/website/cain-map-app/")

# europe <- read_csv("../src/lib/data/server_clean_241007.csv") %>%
#   mutate(date_str = paste0(day_start,str_extract(month, "[a-zA-Z]+"),year),
#          date = dmy(date_str)) 

europe <- readRDS("../cain_map_input_files/server_clean_260116.rds") %>%
...
actors_clean <- read_csv("../cain_map_input_files/all_actors_simpliefied_website_2521204.csv")
...
all_events_filtered <- europe %>%
  ...
  filter(!is.na(town_ln),
         !is.na(town_lat),
         !is.na(date),
         !country_coded%in%c("Spain", "Portugal", "Netherlands")) %>%
...
country_borders <- read_csv("../lib/data/CShapes-2.0.csv") %>%
